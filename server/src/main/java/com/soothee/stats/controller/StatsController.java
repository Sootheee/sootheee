...
public class StatsController {
    ...
    @Parameters(value = {
            @Parameter(name = "year", description = "조회할 년도", example = "/stats/avg?year=2022", required = true, in = ParameterIn.QUERY),
            @Parameter(name = "month", description = "조회할 달", example = "/stats/avg?year=2022&month=2", required = true, in = ParameterIn.QUERY)
    })
    @ApiResponses(value = {
    ...
    public ResponseEntity<?> sendMonthlyAvg(@RequestParam("year") Integer year,
                                            @RequestParam("month") Integer month,
                                            @AuthenticationPrincipal AuthenticatedUser loginInfo) {
        MonthlyAvgDTO result = statsService.getMonthlyAvgInfo(loginInfo, year, month);
        if (result.getDairyCnt() < 5) {
            return new ResponseEntity<>(MyErrorMsg.NOT_ENOUGH_DAIRY_COUNT, HttpStatus.NO_CONTENT);
        }
        return new ResponseEntity<MonthlyAvgDTO>(result, HttpStatus.OK);
    }
}
